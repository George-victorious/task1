...
const PaginationButtonsComponent = ({current, length, setPage}) => {
  let buttons = [];
  ...
  return (
    <PaginationButtons>
      <button onClick={() => (current > 0 ? setPage(current - 1) : null)}>{"<"}</button>
      {buttons}
      <button onClick={() => (current < length - 1 ? setPage(current + 1) : null)}>
        {">"}
      </button>
    </PaginationButtons>
  );
...
const KeywordsCell = ({rowData, ...props}) => {
  ...
  return (
    <Cell {...props}>
      {rowData.keywords.map((keyword) => (
        <Keyword key={keyword} clr={color}>
          {keyword}
        </Keyword>
      ))}
    </Cell>
    ...
    <>
      <Table loading={!posts.length} data={posts}>
        ...
        <Column resizable>
          ...
          <Cell dataKey="origin">{(rowData) => rowData.title}</Cell>
        </Column>
        ...
        <Column resizable>
          ...
          <Cell dataKey="unread">{(rowData) => (rowData.unread ? "Yes" : "No")}</Cell>
        </Column>

        <Column resizable>
          <HeaderCell>Update count</HeaderCell>
          <Cell dataKey="updateCount">{(rowData) => rowData.updateCount ?? 0}</Cell>
        </Column>
      </Table>
